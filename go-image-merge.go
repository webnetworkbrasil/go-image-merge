...
func (m *MergeImage) readGridImage(grid *Grid) (image.Image, error) {
	...
	return m.ReadImageFile(imgPath, grid)
}
...
func (m *MergeImage) ReadImageFile(path string, grid *Grid) (image.Image, error) {
	...
	img = resize.Resize(grid.Width, grid.Height, img, resize.Bilinear)
